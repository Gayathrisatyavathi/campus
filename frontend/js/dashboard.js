...
async function registerTraining(id){
  try{
    const d=await api(`/api/training/${id}/register`,{method:"POST"});
    toast(d.message);
  }catch(e){toast(e.message,"error")}
}
...
window.showSection=showSection;window.loadResource=loadResource;window.applyPlacement=applyPlacement;window.registerTraining=registerTraining;window.startTest=startTest;window.deleteResume=deleteResume;window.addAchievement=addAchievement;
